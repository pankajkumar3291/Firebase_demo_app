package com.pank.pankapp.components;
...
public class PasswordViewWithEye extends AppCompatEditText {
    ...
    public PasswordViewWithEye(Context context) {
        super(context);
        init(null);
    }

    public PasswordViewWithEye(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public PasswordViewWithEye(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(AttributeSet attrs) {
        if (attrs != null) {
            TypedArray typedArray = getContext().getTheme().obtainStyledAttributes(attrs, R.styleable.PasswordView, 0, 0);
            try {
...
//                setVisibility(s.length() > 0 && useStrikeThrough && !visible ? View.VISIBLE : View.GONE);
//            }
//        });
